...
3. Verify you see:
   ...
   > cd equity-registry-contract && npm run build
...
3. npm run build-production
   ...
   3a. cd equity-registry-contract && npm run build
       → Compiles contract
       → Generates/copies keys to equity-registry-contract/dist/managed/

   3b. cd ../frontend-vite-react && npm run build

       3b.1. npm run copy-contract-keys
             → Copies keys from equity-registry-contract/src/managed/
